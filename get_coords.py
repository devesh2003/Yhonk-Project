...
class CoordinateSearch:
    ...
    def start(self):
        self.get_data(self.date,self.date,'05:00:00','05:01:00')
    ...
    def get_slot(self,time):
        times = []
        for i in range(0,24):
            times.append(i)

        data = [str(self.hour_counter)]
        if data[0][0] == '0':
            return times.index(int(data[0].strip('0')))
        else:
            return times.index(int(data[0]))
...
def start(bus,id,dates):
    try:
        ...
        main_data = []
        csv_data = [
            ['Bus Body','Date','Packet','Slot','Latitude','Longitude','Place']
        ]
        for date in dates:
